...
class _CameraPageState extends State<CameraPage> {
  ...
  final double focusBoxHeight = 400.0;
  ...
  String _getPestDescription(String? rawLabel) {
    ...
    return "${info["desc"]}\n\n"
        "How to Identify: ${info["identify"]}\n\n"
        "Possible Damage: ${info["damage"]}\n\n"
        "Immediate Mitigation: ${info["mitigation"]}\n\n"
        "Recommended Management: ${info["recommendation"]}\n\n"
        "Monitoring Advice: ${info["monitoring"]}";
  }
  ...
  Future<File> _cropToFocusBox(File originalFile) async {
    ...
    final focusBoxTop = (screenH - focusBoxHeight) / 4;
    final focusBoxLeft = (screenW - focusBoxWidth) / 2;

    final previewLeft = focusBoxLeft + dx;
    final previewTop = focusBoxTop + dy;
    ...
    int cropW = (focusBoxWidth * scaleX).round();
    int cropH = (focusBoxHeight * scaleY).round();
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          Positioned(
            top: focusBoxTop,
            left: (MediaQuery.of(context).size.width - focusBoxWidth) / 2,
            child: Container(
              width: focusBoxWidth,
          ...
          if (_isImageClassified)
            _DetectionOverlay(
              image: _image,
              label: (_label == "Unclassified")
                  ? "No Clear Pest or Disease Detected"
                  : (_pestInfo[_label]?["title"] ?? _label ?? "Detection Result"),
              confidence: _confidence,
              info: _pestInfo[_label] ?? _pestInfo["Unclassified"]!,
              onClose: _resetClassification,
              primaryButtonText: "Save Detection",
              primaryIcon: Icons.cloud_upload_outlined,
              onPrimaryPressed: _saveData,
            ),
        ],
      ),
    );
